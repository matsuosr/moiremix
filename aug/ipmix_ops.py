"""IPMix ops ported from hzlsaber/IPMix (ImageNet settings).

Mapping:
- IPMix_utils.py: augmentations / augmentations_all / mixings / patch_mixing
"""
import os
import random
...
def sanitize01(x: torch.Tensor) -> torch.Tensor:
...
def patch_mixing(img, mixing_pic, patch_size, mixed_op, beta):
    ...
    img_copy = img.copy()
    patch_left = random.randint(0, org_w - patch_size)
    patch_top = random.randint(0, org_h - patch_size)
    if random.random() > 0.5:
    ...
    mixed = mixed_op(tensorize(img_patch), tensorize(mixing_patch), beta)
    mixed = sanitize01(mixed)
    if os.getenv("IPMIX_DEBUG_FINITE", "0") == "1":
        assert torch.isfinite(mixed).all(), "IPMix mixed patch has non-finite values"
    mixed_patch = imageize(mixed)
    img_copy.paste(mixed_patch, (patch_left, patch_top), mask=None)
    return img_copy
